...
Note, in particular, that when the point $P\ne Y$ is on the line $EY$, it gets assigned the coordinates $(1,m)$ for some $m$. This is so because the first coordinate, that we called $a$ for the general case, comes from the coordinates of $PY\wedge OE$, which in the special case we are analyzing coincides with $E$, whose coordinates are $(1,1)$.
...
\begin{xmpl}\label{xmpl:Moulton-coordinatization} [Moulton coordinatization]
    ...
    We have $O\equiv(0,0)$, $X\equiv\text{horizontal slope}$, $Y\equiv\text{vertical slope}$, and $E\equiv(1,\frac12)$.
...
\begin{proof}
    ...
    \[
        ...
        \quad\psi[1,0]=[1',0'].
    \]
    Additionally, from $OY\colon[0]$, $XY\colon[\infty]$, and $YE\colon[1,0]$, we respectively deduce
    \[
        \psi[0]=[0'],\quad\psi[\infty]=[\infty'],
            \quad\text{and}\quad\psi[1,0]=[1',0'].
    \]
    In consequence,
...
\begin{rem}
    ...
    {\small\alg
        \begin{enumerate}[itemsep=1.8pt, parsep=0pt]
        \setcounter{enumi}{9}
            \item Locate $X=XE\wedge[0,0]$\qquad
            \item Draw $YX$
            \item Locate $Z=YX\wedge OE$
            \item Draw $XZ$
            \item Locate $Y=XZ\wedge YE$
            \item Draw $OY$
            \item Locate $Y=OY\wedge XY$
        \end{enumerate}
    }
    \vspace{-0.7\parskip}
...
It is not generally valid that the operations of sum and multiplication defined on a ternary ring $(\ter,T)$ must satisfy the equality $x\cdot m\+b=T(x,m,b)$. In this section we study these kinds of relations between geometric and algebraic properties and their equivalences.
...
\begin{proof}
    ...
    $$
        ...
        {\footnotesize
        \begin{align*}
            Q'&=BY\wedge OE\colon(b,b)\\
            Q\phantom'&=XQ'\wedge OY\colon(0,b)\\
            P'&=BY\wedge ZC\colon(b,b\+c)\\
            P\phantom'&=XP'\wedge OY\colon(0,b\+c)\\
            R\phantom'&=AY\wedge ZQ\colon(a,a\+b)\\
            R'&=XR\wedge OE\colon(a\+b,a\+b)\\
            S\phantom'&=AY\wedge ZP\colon(a,a\+(b\+c))\\
            S'&=YR\wedge ZC\colon(a\+b,(a\+b)\+c)\\
        \end{align*}
        }
        \end{minipage}
    $$
    ...
    To verify the equality $T(a,b,c\cdot b)=(a\+c)\cdot b$, take an elation $\beta$ with center $X$ and axis $XY$ satisfying $\beta(O)=A$. Since $\beta$ fixes $X$, it fixes the second coordinate of every point, this is, given $x,y\in\ter$, we have $\beta(x,y)=(x',y)$ for some $x'\in\ter$.
